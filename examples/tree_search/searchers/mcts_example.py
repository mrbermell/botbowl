...
from examples.tree_search.searchers.search_util import get_score_sum
...
class MCTS:
    ...
    def __init__(self, game: botbowl.Game, agent, tree_policy, action_policy, heuristic):
        self.game = game
        self.agent = agent
        self.tree_policy = tree_policy
        self.action_policy = action_policy
        self.heuristic = heuristic
        self.scripted_bot = examples.scripted_bot_example.MyScriptedBot('mcts rollout scripted')
        self.scripted_bot.new_game(game, team=game.get_agent_team(agent))
    ...
    def _rollout(self):
        ...
        score_sum = get_score_sum(self.game)
        while not self.game.state.game_over and self.game.state.half == 1 and score_sum == get_score_sum(self.game):
            # action = self.action_policy(self.game, self.game.get_agent_team(self.agent))
            action = self.scripted_bot.act(self.game)
...
class MCTSBot(botbowl.Agent):
    ...
    def act(self, game):
        ...
        if len(game.state.available_actions) == 1 and \
                len(game.state.available_actions[0].positions)==0 and \
                len(game.state.available_actions[0].players)==0:
            action = Action(game.state.available_actions[0].action_type)
            print(f"Only action available: {action}")
            return action
        ...
        print(f"{root.num_visits()}/{len(root.available_actions)} - {action}")
        root.print()
        exit()
